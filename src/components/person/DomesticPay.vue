<template>
  <div class="global" :class="className">
    <topNav title="个人会员付款"></topNav>
    <div class="payMenu">
      <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect">
        <el-menu-item index="1">全球个人会员</el-menu-item>
        <el-menu-item index="2" >国内个人会员</el-menu-item>
      </el-menu>
    </div>
    <div class="payBody">
      <div class="paySection" style="margin-top:0">
        ...
        <div class="payContent">
          ...
          <div class="contentLine" style="padding-bottom: 20px; margin-left: -10px;">
            <div v-for="(item,index) in rechageData" :key="index" 
              class="payItem payItem1" :class="{selected: rechargeValue===item.money}" 
              @click="chooseRecharge(item.money)">
              <p>{{item.money}}金币</p>
              <p>{{item.money}}元</p>
              <label class="ibg">
        ...
        <div class="payContent">
          ...
          <div class="contentLine" style="padding-bottom: 20px; margin-left: -10px;">
            <div v-for="(item,index) in vipData" :key="index" 
              class="payItem payItem2" :class="{selected: vipValue===item.money*item.amount}" 
              @click="chooseVip(item.money*item.amount)">
              <p>{{item.money}}<span>人民币/月</span></p>
          ...
          <div class="contentLine">
            ...
            <div class="reminding">注：国内个人会员付费期间可与国内会员/非会员免费通信</div>
          </div>
        </div>
      </div>
...
export default {
  data(){
    return {
      activeIndex: '2',
      rechageData: [
        { money: 10},{ money: 30},{ money: 50},
        { money: 70},{ money: 90},{ money: 100},
      ],
      vipData: [
        { money: 99, amount: 1, text: '1个月'},
        { money: 69, amount: 6, text: '6个月'},
        { money: 49, amount: 12, text: '1年'}
      ],
      payWayData: [
        { text: "支付宝", value: "zfb" },
        { text: "微信支付", value: "wx" },
  ...
  computed:{
    ...
    total(){
      return this.rechargeValue + this.vipValue;
    },
    className(){
  ...
  methods: {
    ...
    submit(){
      if (this.rechargeValue === 0 && this.vipValue === 0) {
        this.$message({
          message: `请选择充值内容！`,
          showClose: true,
          type: "error"
      ...
      this.$message({
        message: `${this.rechargeValue}-${this.vipValue}-${this.payWayValue}`,
        showClose: true
      })
    }
  },
