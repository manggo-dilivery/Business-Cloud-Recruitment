<template>
    <div class="container">
        <topNav title="企业注册"></topNav>
        <div class="infoList">
            <div class="infoItem">
                <input v-model="id" type="text" name="name" placeholder="请输入公司注册id">
            ...
            <div class="infoItem">
                <input v-model="name" type="text"  placeholder="请输入公司名称">
                <img src="@/static/image/login/inputCancel.png" @click="cancelName" alt="">
            </div>
            <div class="infoItem">
            ...
            <div class="infoItem">
                <input v-model="invatationCode" type="text" name="name" placeholder="请输入邀请码（选填）">
...
<script>
    ...
    methods:{
      ...
      goNext(){
        ...
        if(id==''){
          Toast({message:"公司id不能为空", position:top, duration:3000});
          return
        };
        ...
        if(userName==''){
          Toast({message:"公司名称不能为空", position:top, duration:3000});
          return
        };
        ...
        let info={'id':id,'password':password,'userName':userName,'email':email,'code':code,'place':place}
        // window.localStorage.setItem('companyRegist',JSON.stringify(info))
        // this.$router.push('/companyRegister2')
        this.$axios.post('/business/pass/save?area='+info.place+'&companyName='+info.userName+'&email='
          +info.email+'&password='+info.password+'&pid=0&type=1&username='+info.id+'&invitedCode='+info.code
